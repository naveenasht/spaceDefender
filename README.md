...
- **Levels** — five sectors (Alpha through Epsilon) scaling enemy spawn rate, shooter/elite mix, and a survival-time goal from 1:30 up to 3:30, shown as a countdown bar under the HULL bar and as a 5-dot difficulty rating on each card — reach the goal and the sector clears; run out of lives first and it's Game Over instead. Only 3 cards show at once; arrow keys/A-D scroll the window (wrapping around at the ends) and a "SECTOR N OF 5" label tracks where you are.
